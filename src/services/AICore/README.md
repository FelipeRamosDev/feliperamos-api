...
├── AICoreTurn.ts      # Response handling and streaming
...
##### `response(model?: AIModels, systemPrompt?: string): AICoreTurn`
...
##### `options(options: ResponseCreateParamsNonStreaming | ResponseCreateParamsStreaming): AICoreTurn`
Sets OpenAI API options.

##### `model(model: AIModels): AICoreTurn`
Sets the AI model for this response.

##### `systemPrompt(systemPrompt: string): AICoreTurn`
