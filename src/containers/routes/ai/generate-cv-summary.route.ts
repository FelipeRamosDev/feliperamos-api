...
export default new EventEndpoint({
   ...
   controller: (params: GenerateSummaryParams = {}, done = () => {}) => {
      const { jobDescription = '', threadID, additionalContext = '', currentInput = '' } = params || {};

      if (!(service instanceof AI)) {
         return done(new ErrorEventEndpoint(`The AI service is not available!`, 'AI_SERVICE_NOT_AVAILABLE'));
      }

      const prompt = generateCVSummaryPrompt(jobDescription, currentInput, additionalContext);
      service.threadMessage(threadID, prompt).then(({ threadID: newThreadID, output }: any) => {
         done({
            success: true,
